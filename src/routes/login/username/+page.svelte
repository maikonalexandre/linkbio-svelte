<script lang="ts">
    ...
    async function configUsername() {
        ...
        bacth.set(doc(db, "usernames", username), { uid: $user?.uid });
        bacth.set(doc(db, "users", username), {
            username,
...
<AuthCheck>
    <h2>Username</h2>
    <form on:submit|preventDefault={configUsername} class="w-2/5">
        <input
            placeholder="username"
            type="text"
            class="input w-full"
            bind:value={username}
            on:input={checkIsAvailable}
